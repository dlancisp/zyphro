...
export default function SecureDrop() {
  ...
  const fetchAndDecryptMessage = async (id, key) => {
    ...
    try {
      ...
      if (!res.ok) throw new Error("Mensaje destruido.");
  ...
  return (
    <div className="min-h-screen bg-[#020617] text-white font-sans selection:bg-blue-500/30 overflow-x-hidden">
      ...
      {/* NAVBAR UNIFICADO (Estilo Home) */}
      <nav className="max-w-7xl mx-auto px-6 h-24 flex justify-between items-center relative z-[100]">
        <Link to="/" className="flex items-center">
          <span style={{ fontSize: '1.875rem', fontWeight: '900', fontStyle: 'italic', letterSpacing: '-0.05em', textTransform: 'uppercase', color: '#2563eb', paddingRight: '0.4em' }}>
            ZYPHRO
          </span>
        </Link>
        ...
        <div className="w-full max-w-xl bg-slate-900/40 backdrop-blur-3xl rounded-[2.5rem] shadow-2xl border border-white/5 overflow-hidden relative">
          {loading && (
            <div className="absolute inset-0 bg-[#020617]/80 backdrop-blur-sm z-50 flex flex-col items-center justify-center">
              ...
              <span className="text-[10px] font-black tracking-[0.3em] text-blue-500 uppercase">Cifrando...</span>
            </div>
          )}
          ...
          <div className="p-8 border-b border-white/5 flex justify-between items-center bg-white/5 text-blue-600 font-black italic uppercase tracking-tighter">
            ...
            <button onClick={() => window.location.href='/drop'} className="p-2 text-slate-500 hover:text-blue-500"><RotateCcw size={18} /></button>
          </div>
              ...
              !link ? (
                <div className="space-y-6">
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <button onClick={() => setViewOnce(true)} className={`p-6 rounded-2xl border flex flex-col items-center gap-3 transition-all ${viewOnce ? 'border-blue-600 bg-blue-600/10' : 'border-white/5 opacity-50'}`}>
                      <Eye size={24} className={viewOnce ? 'text-blue-500' : 'text-slate-500'} /><span className="text-[9px] font-black uppercase">Un solo uso</span>
                    </button>
                    <button onClick={() => setViewOnce(false)} className={`p-6 rounded-2xl border flex flex-col items-center gap-3 transition-all ${!viewOnce ? 'border-blue-600 bg-blue-600/10' : 'border-white/5 opacity-50'}`}>
                      <Clock size={24} className={!viewOnce ? 'text-blue-500' : 'text-slate-500'} /><span className="text-[9px] font-black uppercase">24 Horas</span>
                    </button>
                  </div>
                ...
                <div className="text-center space-y-8 py-4">
                  ...
                  <div className="bg-black/60 p-6 rounded-3xl border border-white/5">
                    ...
                    <button onClick={() => {navigator.clipboard.writeText(link); toast.success("Copiado!")}} className="w-full bg-blue-600 text-white py-3 rounded-xl font-black text-[10px] uppercase">Copiar Enlace</button>
                  </div>
                </div>
              )
            ) : (
              <div className="space-y-8">
                <div className="flex justify-center">{incomingMsg.viewOnce ? <div className="text-amber-500 font-black text-[10px] uppercase bg-amber-500/10 px-6 py-3 rounded-2xl border border-amber-500/20"><Trash2 size={14} /> Último Acceso</div> : <CountdownTimer expiresAt={incomingMsg.expiresAt} />}</div>
